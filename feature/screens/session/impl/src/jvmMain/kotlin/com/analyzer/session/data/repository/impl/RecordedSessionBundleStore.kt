...
internal class RecordedSessionBundleStore(private val settings: TelemetryAcquisitionSettings, private val json: Json) {
    ...
    suspend fun loadSummaries(forceRefresh: Boolean = false): List<RecordedSessionSummary> = resolveRoot()
        ...
        ?.map(SessionBundleLocation::summary)
        .orEmpty()
    ...
    private fun rebuildIndex(root: File, fingerprint: RootFingerprint): BundleIndexCache {
        ...
        duplicatedRuntimeSessionIds.forEach { (runtimeSessionId, collisions) ->
            logger.debug {
                "duplicate runtime sessionId=$runtimeSessionId detected (${collisions.size} entries), keeping all entries via stable persisted IDs"
            }
        }
    ...
    private fun peekSharedCache(rootPath: String, fingerprint: RootFingerprint): BundleIndexCache? =
        synchronized(cacheLock) {
            sharedCache?.takeIf { cache ->
    ...
    private data class RootFingerprint(
        val rootLastModified: Long,
        val directoryCount: Int,
        val directoryHash: Long,
    )

    private data class BundleIndexCache(
        val rootPath: String,
    ...
    private companion object {
        ...
        private var sharedCache: BundleIndexCache? = null
    }
}
