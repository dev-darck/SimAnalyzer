...
class CalibrationViewModel(
    ...
    fun dispatch(intent: CalibrationIntent) {
        when (intent) {
            ...
            is CalibrationIntent.SetFileToSave -> onSetFileToSave(intent.path)
            CalibrationIntent.LoadAllFiles -> onLoadAllFiles()
    ...
    private fun onSetFileToSave(path: String) {
        saveUseCase.setPath(path)
        _state.update { it.copy(savedPathHint = path) }
        dispatch(CalibrationIntent.LoadAllFiles)
    }

    private fun onLoadAllFiles() {
        viewModelScope.launch {
            val listOfData = saveUseCase.loadAll()
            if (listOfData == null) {
                _state.update { it.copy(savedPathHint = "Please select path for saving") }
            } else {
    ...
    private fun save() {
        ...
        viewModelScope.launch {
            _state.update { it.copy(isBusy = true, message = "⏳ Saving to ${it.savedPathHint}…") }
            try {
